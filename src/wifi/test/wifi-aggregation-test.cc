...
AmpduAggregationTest::DoRun (void)
{
  ...
  NS_TEST_EXPECT_MSG_EQ (m_low->m_aggregateQueue->GetNPackets (), 0, "aggregation queue is not flushed");
  ...
  m_edca->GetEdcaQueue ()->PushBack (Create<WifiMacQueueItem> (pkt1, hdr1));
  m_edca->GetEdcaQueue ()->PushBack (Create<WifiMacQueueItem> (pkt2, hdr2));

  isAmpdu = m_low->IsAmpdu (pkt, hdr);
  uint32_t aggregationQueueSize = m_low->m_aggregateQueue->GetNPackets ();
  ...
  NS_TEST_EXPECT_MSG_EQ (m_edca->GetEdcaQueue ()->GetNPackets (), 0, "queue should be empty");
  ...
  {
    dequeuedItem = StaticCast<WifiMacQueueItem> (m_low->m_aggregateQueue->PopFront ());
  ...
  Ptr<WifiMacQueueItem> item3 = Create<WifiMacQueueItem> (pkt3, hdr3);
  m_edca->GetEdcaQueue ()->PushBack (item3);
  ...
  m_edca->GetEdcaQueue ()->Remove (item3);
...
TwoLevelAggregationTest::DoRun (void)
{
   ...
   */
  m_edca->GetEdcaQueue ()->PushBack (Create<WifiMacQueueItem> (pkt, hdr));
  m_edca->GetEdcaQueue ()->PushBack (Create<WifiMacQueueItem> (pkt, hdr));

  Ptr<const WifiMacQueueItem> peekedItem = m_edca->GetEdcaQueue ()->PeekByTidAndAddress (0, WifiMacHeader::ADDR1, hdr.GetAddr1 ());
  peekedHdr = peekedItem->GetHeader ();
  tstamp = peekedItem->GetTimeStamp ();
  m_low->m_currentPacket = peekedItem->GetConstPacket ()->Copy ();
  m_low->m_currentHdr = peekedHdr;
  m_low->m_currentTxVector = m_low->GetDataTxVector (m_low->m_currentPacket, &m_low->m_currentHdr);

  Ptr<Packet> packet = m_low->PerformMsduAggregation (peekedItem->GetConstPacket (), &peekedHdr, &tstamp, currentAggregatedPacket, 0);
  ...
  NS_TEST_EXPECT_MSG_EQ (m_edca->GetEdcaQueue ()->GetNPackets (), 0, "aggregated packets not removed from the queue");
  ...
  m_edca->GetEdcaQueue ()->PushBack (Create<WifiMacQueueItem> (pkt, hdr));
  packet = m_low->PerformMsduAggregation (peekedItem->GetConstPacket (), &peekedHdr, &tstamp, currentAggregatedPacket, 0);
  ...
  m_edca->GetEdcaQueue ()->RemovePacket (pkt);
  m_edca->GetEdcaQueue ()->RemovePacket (pkt);
  packet = m_low->PerformMsduAggregation (peekedItem->GetConstPacket (), &peekedHdr, &tstamp, currentAggregatedPacket, 0);
